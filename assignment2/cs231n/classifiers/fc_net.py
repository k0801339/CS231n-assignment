...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if use_batchnorm:
            self.params['gamma1'] = np.ones(hidden_dims[0]) #initial to one
            self.params['beta1'] = np.zeros(hidden_dims[0]) #initial to zero
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        #產生紀錄hidden_layer, caches, dropout_caches 之 array
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:  #means it's the output layer, do no dropout/batch norm
        ...
        for i in range(self.num_layers, 0, -1): #i from num_layers to '1'
            if i == self.num_layers:
                ...
                if self.use_batchnorm:
                    ...
                    dhiddens[i-1], grads['W'+str(i)], grads['b'+str(i)] = affine_relu_backward(dhiddens[i], caches[i-1])
            #count loss, remember to add L2 regularization
