...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x_normalized, sample_mean, sample_var, gamma, beta, x, eps)
        running_mean = momentum* running_mean + (1-momentum) * sample_mean
        running_var = momentum * running_var + (1-momentum) * sample_var
        #######################################################################
        #                           END OF YOUR CODE                          #
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x_normalized, sample_mean, sample_var, gamma, beta, x, eps = cache
    N, D = x.shape
    dx_normalized = dout * gamma #(N, D)
    x_mu = x - sample_mean #(N, D)
    sample_std_inv  = 1.0 / np.sqrt(sample_var + eps) #(1, D)
    dsample_var = np.sum(-0.5 * sample_std_inv**3 * dx_normalized * x_mu, axis=0, keepdims=True)
    ...
    dx = dx_normalized * sample_std_inv + dsample_var * 2 * x_mu / N + dsample_mean / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = out + (b)[None,:,None,None] #add bias only to out.shape[1]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def max_pool_forward_naive(x, pool_param):
    ...
    output_H, output_W = H // pool_height, W // pool_width
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(output_H):
        for j in range(output_W):
            mask = x[:,:,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width]
            m = np.max(mask, axis=(2,3), keepdims=True)
            flags = (mask == m)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_new = x.transpose(0, 2, 3, 1).reshape(N*H*W, C) #transpose x to get suitable shape for bn operation
